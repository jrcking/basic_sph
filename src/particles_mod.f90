module particles_mod
  ...
  real, allocatable,dimension(:) :: m,ro,p,c,drodt
  real, allocatable,dimension(:,:) :: v,r,dvdt,drdt
  real, allocatable,dimension(:,:,:) :: tau
  integer,allocatable,dimension(:) :: nneigh,mp_identity
  ...
  real :: h,nu,mu,ro_init,av_value,xsph,h_ov_spacing
  integer :: n,ng
  logical,dimension(dims) :: wbc
  logical :: kgc,nnf,slip,t_corr,compressible
...
contains
  ...
  subroutine create_particles
    ...
    allocate(drdt(n,dims))
    allocate(p(n+ng))
    allocate(c(n+ng))
    allocate(nneigh(n+ng))
    allocate(mp_identity(ng))
    allocate(neigh_list(n+ng,n+ng))
    allocate(tau(n+ng,dims,dims))
